...
if (ret) {
    ...
    // This is a cache miss
    erasureWriteCount++;
    stat.cache[1]++;  // Increment total write count
}

// After the 1-second window, calculate FEIO
uint64_t currentTime = tick;
if (currentTime >= (startTime + 1000000000)) {  // Assuming tick is in nanoseconds
    double feio = static_cast<double>(erasureWriteCount) / totalWriteCount;

    // Prevent the compiler from optimizing 'feio' away
    std::cout << "FEIO: " << feio << std::endl;

    // Reset counters for the next time window
    totalWriteCount = 0;
    erasureWriteCount = 0;
    startTime = currentTime;  // Update the starting time for the next window
} 
//FEIO ends
...
//For erasure IOs
  temp.name = prefix + "generic_cache.read.erasureIOs";
  temp.desc = "Erasure IO count";
  list.push_back(temp);


 //For FEIO (Fraction of Erasure IOs)
    temp.name = prefix + "generic_cache.write.feio";
    temp.desc = "Fraction of Erasure IOs";
    list.push_back(temp); 
...
// Calculate FEIO
double feio_val = totalWriteCount == 0 ? 0 : static_cast<double>(erasureIOs) / totalWriteCount;

// Push the FEIO value into the vector
values.push_back(feio_val);

// Print or use the FEIO value
std::cout << "FEIO: " << feio_val << std::endl;
