...
namespace ICL {
  ...
  uint64_t lastAccessedAddress;
  uint64_t lastAccessedTime;
  uint64_t lastErasureCheckTime;
  const uint64_t erasureCheckInterval = 1000000000;  // 1 second in ticks
  uint64_t erasureIOs = 0;
  
  const uint64_t T = 1000000000; // 1 second in nanoseconds
const uint64_t tickInterval = 1000; // Assuming tick is in nanoseconds
...
// Placeholders for FEIO calculation
int erasureWriteCount = 0;
int totalWriteCount = 0;
uint64_t erasureStartTime = tick;

// Add these global variables for (short slope and long slope)
uint64_t erasuresTotalCurrentTimeSlice = 0;
uint64_t erasuresTotalPreviousTimeSlice = 0;
uint64_t timeSliceTotalStartTime = 0;
...
//value
 ...
 void checkErasure(Request& req, uint64_t tick) {
  uint64_t currentTime = tick;
    // Convert tick to seconds
    uint64_t tickInSeconds = tick / 1e9;
 

    // Check if 1 second has passed since the last erasure check
    if (tickInSeconds - lastErasureCheckTime >= erasureCheckInterval) {
        // Check if the last accessed address is not zero (indicating a valid previous access)
        if (lastAccessedAddress != 0) {
            // Check if the last accessed address matches the current request's address
            if (lastAccessedAddress == req.range.slpn) {
                // Check if the time difference is within the specified threshold (T)
                if (tick - lastAccessedTime <= T) {
                    debugprint(LOG_ICL_GENERIC_CACHE, "Erasure IO detected!");
                    // Update erasure IO count when erasure occurs
                    erasureIOs++;
                }
            }
            // Reset last accessed address and time
        lastAccessedAddress = 0;
        lastAccessedTime = 0;
        }

        // Update last erasure check time
        lastErasureCheckTime = tickInSeconds;
    }

    // Update last accessed address and time for each request
    lastAccessedAddress = req.range.slpn;
    lastAccessedTime = tick;

    //short and long slope
      if (currentTime - lastErasureCheckTime >= erasureCheckInterval) {

        double shortSlopeTotal = calculateShortSlope(erasuresTotalCurrentTimeSlice, erasuresTotalPreviousTimeSlice);
        std::cout << "SHORTSLOPE: " << shortSlopeTotal << std::endl;

        double longSlopeTotal = calculateLongSlope(erasuresTotalCurrentTimeSlice, timeSliceTotalStartTime, currentTime);
        std::cout << "LONGSLOPE: " << longSlopeTotal << std::endl;

        erasuresTotalPreviousTimeSlice = erasuresTotalCurrentTimeSlice;
        timeSliceTotalStartTime = currentTime;
        lastErasureCheckTime = currentTime;
    }

}

// longslope and shortslope

double GenericCache::calculateShortSlope(uint64_t currentErasures, uint64_t previousErasures) {
    if (previousErasures == 0) {
        return 0; 
    }
    return static_cast<double>(currentErasures) / previousErasures;
}

double GenericCache::calculateLongSlope(uint64_t currentErasures, uint64_t startTime, uint64_t currentTime) {
    uint64_t elapsedTime = currentTime - startTime;
    if (elapsedTime == 0) {
        return 0; 
    }
    return static_cast<double>(currentErasures) / elapsedTime;
}
...
bool GenericCache::read(Request &req, uint64_t &tick) {
  ...
  if (ret) {
    stat.cache[0]++;
     ...
     // Update last accessed address and time
        lastAccessedAddress = req.range.slpn;
        lastAccessedTime = tick;


   // Check for erasure logic
  checkErasure(req, tick);
  acceio_read++;
    ...
    if (wayIdx != waySize) {
      ...
      // We have to flush
        ...
        for (setIdx = 0; setIdx < setSize; setIdx++) {
          ...
          if (ret) {
            // This is a cache hit
            totalWriteCount++;  // Increment total write count
        } else {
            // This is a cache miss
            totalWriteCount++;  // Increment total write count
            erasureWriteCount++;  // Increment erasure write count
        }
      ...
      debugprint(LOG_ICL_GENERIC_CACHE,
         ...
         // Check if the current time has exceeded the 1-second window
        uint64_t currentTime = tick;
        if (currentTime - erasureStartTime >= 1000000000) {  // 1 second in nanoseconds (in tick)
            // Calculate and print FEIO
            double feio = erasureWriteCount == 0 ? 0 : static_cast<double>(erasureWriteCount) / totalWriteCount;
            std::cout << "FEIO: " << feio << std::endl;

            // Reset counters for the next 1-second window
            erasureWriteCount = 0;
            totalWriteCount = 0;
            erasureStartTime = currentTime;
        }
...
//For erasure IOs
  temp.name = prefix + "generic_cache.erasureIOs";
  temp.desc = "Erasure IO count";
  list.push_back(temp);

//FEIO- fraction of erasure IOs over the total number of writes for a time slice(i.e., 1 second)


 

// Additional stats for AVEIO
  temp.name = prefix + "generic_cache.aveio";
  temp.desc = "Average length of continuously erased IOs";
  list.push_back(temp);
 
//ACCEIO
  temp.name = prefix + "generic_cache.acceio";
  temp.desc = "Accumulated erasure IOs";
  list.push_back(temp);

// SHORTSLOPE

    temp.name = prefix + "generic_cache.shortslope";
    temp.desc = "Combined SHORTSLOPE for reads and writes";
    list.push_back(temp);

//LONGSLOPE

   temp.name = prefix + "generic_cache.longslope";
    temp.desc = "Combined LONGSLOPE for reads and writes";
    list.push_back(temp);
...
std::cout << "EIO: " << erasureIOs << std::endl;
  // Use the AVEIO values within the function
  // For example, you can print the values to verify the calculations
  std::cout << "AVEIO: " << aveio_read_val << std::endl;
  

  //FEIO
    // Calculate and push FEIO values
    double feio_read_val = (stat.cache[0] == 0) ? 0 : static_cast<double>(stat.erasureIOs[0]) / stat.cache[0];
    values.push_back(feio_read_val);

    // Print FEIO value for verification
    std::cout << "FEIO Read: " << feio_read_val << std::endl;
...
if (currentTime - acceioStartTime >= 10) {
  ...
  std::cout << "ACCEIO: " << acceio_read << std::endl;
 ...
 // Calculate SHORTSLOPE and LONGSLOPE
    double shortslope = calculateShortSlope(erasuresTotalCurrentTimeSlice, erasuresTotalPreviousTimeSlice);
    double longslope = calculateLongSlope(erasuresTotalCurrentTimeSlice, timeSliceTotalStartTime, currentTime);

    // Print SHORTSLOPE and LONGSLOPE
    std::cout << "SHORTSLOPE: " << shortslope << std::endl;
    std::cout << "LONGSLOPE: " << longslope << std::endl;

    // Push SHORTSLOPE and LONGSLOPE values
    values.push_back(shortslope);
    values.push_back(longslope);
//end
